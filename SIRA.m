...
% Last Modified by GUIDE v2.5 03-Apr-2018 19:54:43
...
if(query_filename ~= 0)
    ...
    if(strcmp(lower(ext), '.jpg') == 1 || strcmp(lower(ext), '.png') == 1 || strcmp(lower(ext), '.bmp') == 1)
        ...
        query_image_feature = [color_moments auto_correlogram hsv_histogram mean_amplitude ms_energy wavelet_moments str2double(name)];
        
        handles.query_image_feature = query_image_feature;
        guidata(hObject, handles);
        
        helpdlg('Click on Search button to find simmilar images');
        ...
        errordlg('Please select correct file type');
    end
else
    return;
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
folder_name = uigetdir(pwd,'Select Directory for the Images');
if( folder_name ~= 0 )
    handles.folder_name = folder_name;
    guidata(hObject,handles);
else
    errordlg('Please select the image dirctory first!');
    return;
end
...
if (~isfield(handles,'query_image_feature'))
    errordlg('Please select image first, then choose your similarity feature and no of images!');
    return;
end
